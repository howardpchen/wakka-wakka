# App Store upgrade preparation status

## Prepared

- [x] Original launcher/menu icon source and Pebble-sized icon
- [x] 144×144 large and current 80×80 small App Store icons
- [x] 48×48 legacy small App Store icon
- [x] Icon compiled into both Emery and Basalt payloads
- [x] Icon visually checked in the Emery launcher
- [x] Candidate home and gameplay screenshots
- [x] Physical Time Steel home screenshot from exact 1.1.0 candidate
- [x] Short and long descriptions
- [x] Controls and compatibility text
- [x] Version plan and release notes
- [x] Privacy statement
- [x] Originality/branding review
- [x] Support-information template

## Public information

- [x] Public support email: `acupojava@yahoo.com`
- [x] No support URL or issue tracker
- [x] Public publisher/developer display name: `acupojava`
- [x] Public first version: `1.0.0`
- [x] Upgrade version: `1.1.0`

## 1.0.0 release evidence

- [x] Existing geometric game artwork accepted by hc
- [x] New screenshots from the `1.0.0` release candidate
- [x] Extended physical stability and gameplay test accepted by hc
- [x] Exact `1.0.0` submission PBW installed on physical Pebble Time 2
- [x] Original Emery-only `1.0.0` PBW preserved with its historical checksum

## 1.1.0 upgrade evidence

- [x] Dual-platform source targets Emery and Basalt
- [x] Exact `1.1.0` upgrade PBW built and validated
- [x] Exact `1.1.0` upgrade PBW installed on physical Time Steel
- [x] Time Steel buttons and generally smooth animation accepted by hc
- [x] Revised compact Basalt ready typography visually accepted by hc
- [x] Revised compact Basalt start typography visually accepted by hc
- [x] Basalt home screenshot captured from physical Time Steel
- [x] GitHub commit, remote SHA, and CI verified for the 1.1.0 candidate
- [x] Upgrade entered and reviewed in the developer portal
- [x] Version `1.1.0` published with Emery and Basalt platform support
- [x] Basalt listing screenshot uploaded from the physical Time Steel

## Published listing

- App ID: `d7730c67fb89412e9c655cfd`
- Storefront: <https://apps.repebble.com/d7730c67fb89412e9c655cfd>
- Published package version: `1.1.0`
- Published platforms: `emery`, `basalt`
